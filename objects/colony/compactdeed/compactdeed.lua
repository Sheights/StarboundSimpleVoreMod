...
function onInteraction(args)
  ...
  if isOccupied() then
    if isRentDue() then
      ...
      if self.npcsDeliverRent and primary and world.callScriptedEntity(primary, "tenant.canDeliverRent") then
        world.callScriptedEntity(primary, "tenant.deliverRent", rent.pool, getRentLevel())
      else
        world.spawnTreasure(self.position, rent.pool, getRentLevel())
