...
function init()
	...
	elseif effect.duration() >= 10000 then
		status.addEphemeralEffect("intents", 180.0)
		status.addEphemeralEffect("nofalldamage", 180.0)
		status.addEphemeralEffect("dragonacid", 180.0)
		status.addEphemeralEffect("paralysis", 180.0)		
	...
	if world.entityExists(effect.duration()) then
		...
		effect.modifyDuration(180 - effect.duration())
	else
		request = true
		uninit()
	end	
...
function update(dt)
	...
	if effect.duration() <= 180 and request then
