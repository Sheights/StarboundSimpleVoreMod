...
require "/scripts/vsopathutil.lua"
...
function PathFinder:start(sourcePosition, targetPosition)
  ...
  local baseParameters = mParams()
  local jumpSpeed = baseParameters.airJumpProfile.jumpSpeed
  jumpSpeed = jumpSpeed + ( jumpSpeed * mJumpModifyer() )
...
function PathFinder:canPathfind()
  return mcontroller.onGround() or not mParams().gravityEnabled
end
...
function PathFinder:update(targetPosition)
  ...
  if self:hasPath() and self.stuckTimer > 2.0 then
    self:reset()
  end
...
function PathMover:new(options)
  options = options or {}
  local newPather = {}  
  --parseArgs became applyDefaults
  local pathOptions = applyDefaults(options.pathOptions or {}, {
    returnBest = false,
    mustEndOnGround = mParams().gravityEnabled,
    maxDistance = 200,
    swimCost = 5,
    dropCost = 2,
    boundBox = mBoundBox(),
...
function PathMover:move(targetPosition, dt)
  ...
  if config.getParameter("pathing.forceWalkingBackwards", false) then
    if run == true then run = mMovingFacing() == mFacingDirection() end
  end
...
function PathMover:openDoors()
  local bounds = rect.translate(mBoundBox(), mcontroller.position())
...
function PathMover:moveJump()
  ...
  if world.magnitude(mcontroller.position(), self.edge.source.position) < 1.0 then
    ...
    if mLiquidCheck() or self.jumpTimer <= 0 then
      ...
      mSetVelocity({self.edge.jumpVelocity[1], self.edge.jumpVelocity[2]})
      self.jumpTimer = nil
      self:advancePath()
    else
...
function PathMover:moveArc()
  ...
  if mcontroller.onGround() and not mLiquidCheck() then
    local nextEdge = self.finder:lookAhead(1) or {}
    ...
    mControlApproachXVel(velocity[1], mParams().groundForce)

    if mLiquidCheck() then
...
function PathMover:moveLand()
  if (mcontroller.onGround() or (mLiquidCheck() and math.abs(self.delta[2]) < 1)) and math.abs(self.delta[1]) < 1 then
    self:advancePath()
    mControlApproachXVel(0, mParams().groundForce)
  end
  return "running"
end
  ...
  while self.edge and self.edge.action == "Walk" do
    if passedTarget(self.edge) then
      if self.edge.target.velocity then
        mSetVelocity(self.edge.target.velocity)
      end
      self:advancePath()
    else
      break
  ...
  if self.edge and self.edge.action == "Walk" then
    ...
    for i = 1, 5 do
      ...
      if edge and edge.action == "Walk" then
        if edge.target.velocity and math.abs(edge.target.velocity[1]) > mParams().walkSpeed then
          run = true
        end
      else
        break
