		...
		if world.getProperty("preyEnabled") then
			...
			if #targets == 1 then
				...
				if #people == 1 then
					self.active = true
					self.prey = people[1]
					world.spawnProjectile( "rptool2projectileegg" , world.entityPosition( targets[1] ), entity.id(), {0, 0}, false )
				end
			end
		end
...
function deactivate()
	...
	world.spawnProjectile( "eggcleanser" , mcontroller.position(), entity.id(), {0, 0}, false )
end